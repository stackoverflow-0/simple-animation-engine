    ...
    if (import_animation == true) {
        ...
        for (int k = 0; k < blend_anim_num; k++) {
            for (int i = 0; i < bone_num; i++) {
                ...
                // left frame
                vec4 ca_l = texelFetch(bone_current_poses[k], ivec2((bone_offset    ), frame_id), 0);
                vec4 cb_l = texelFetch(bone_current_poses[k], ivec2((bone_offset + 1), frame_id), 0);
                vec4 cc_l = texelFetch(bone_current_poses[k], ivec2((bone_offset + 2), frame_id), 0);
                vec4 cd_l = texelFetch(bone_current_poses[k], ivec2((bone_offset + 3), frame_id), 0);
                // right frame
                vec4 ca_r = texelFetch(bone_current_poses[k], ivec2((bone_offset    ), frame_id + 1), 0);
                vec4 cb_r = texelFetch(bone_current_poses[k], ivec2((bone_offset + 1), frame_id + 1), 0);
                vec4 cc_r = texelFetch(bone_current_poses[k], ivec2((bone_offset + 2), frame_id + 1), 0);
                vec4 cd_r = texelFetch(bone_current_poses[k], ivec2((bone_offset + 3), frame_id + 1), 0);
            ...
            current_mat = blend_weights[k] * current_mat / total_weight;
        }
